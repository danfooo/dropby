...
Before deploying a change that touches the `Dockerfile` or native dependencies, verify it builds and boots locally first. This machine uses Apple's `container` CLI, not Docker — see the `verify-container-image` skill (`.claude/skills/verify-container-image/SKILL.md`).
...
Dev builds need the APNs sandbox host; TestFlight and production need it unset. See the `apns-sandbox` skill (`.claude/skills/apns-sandbox/SKILL.md`).
