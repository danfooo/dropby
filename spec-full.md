...
- API contract: request body schemas and response types live in the `shared/` workspace (`@dropby/shared`). The server validates every request body against its schema before the handler runs; a malformed body gets 400 `{ error: 'INVALID_REQUEST', issues }`. Schemas check shape and type only and allow extra keys; business rules and their specific error codes stay in the handlers
